...
const child_process_1 = require("child_process"); // For Git suggestions
function activate(context) {
    // 1) Gather Comments
    const gatherCommentsCommand = vscode.commands.registerCommand("projectHelper.gatherComments", () => __awaiter(this, void 0, void 0, function* () {
        try {
            const files = yield vscode.workspace.findFiles("**/*.{js,ts,jsx,tsx,java,py,html,css,cpp,c,cs,php,rb,go}", "**/node_modules/**");
            ...
            const panel = vscode.window.createWebviewPanel("comments", "Project Comments", vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    ...
    // 2) Floating Notes
    const openNotesCommand = vscode.commands.registerCommand("projectHelper.openFloatingNotes", () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage("No active editor found!");
            return;
        }
        const panel = vscode.window.createWebviewPanel("floatingNotes", "Floating Notes", vscode.ViewColumn.Active, { enableScripts: true, retainContextWhenHidden: true });
        const existingNote = context.globalState.get("floatingNote") || { content: "" };
        panel.webview.html = getFloatingNotesHTML(existingNote.content);
        // Handle messages from the WebView
        panel.webview.onDidReceiveMessage((message) => __awaiter(this, void 0, void 0, function* () {
            switch (message.command) {
                case "saveNote":
                    yield context.globalState.update("floatingNote", { content: message.content });
                    vscode.window.showInformationMessage("Your note was saved!");
                    break;
                case "clearNote":
                    yield context.globalState.update("floatingNote", { content: "" });
    ...
    // 3) Search Logs
    const searchLogsCommand = vscode.commands.registerCommand("projectHelper.searchLogs", () => __awaiter(this, void 0, void 0, function* () {
        try {
            const files = yield vscode.workspace.findFiles("**/*.{js,ts,jsx,tsx,java,py,html,css,cpp,c,cs,php,rb,go}", "**/node_modules/**");
            const logRegexList = [
                /console\.log\(.*?\)/g,
            ...
            const panel = vscode.window.createWebviewPanel("logs", "Project Logs", vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
    ...
    const insertLogSnippetCommand = vscode.commands.registerCommand("projectHelper.insertLogSnippet", () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage("No active editor found!");
            return;
    ...
    // 5) Search TODOs
    const searchTodosCommand = vscode.commands.registerCommand("projectHelper.searchTodos", () => __awaiter(this, void 0, void 0, function* () {
        try {
            const files = yield vscode.workspace.findFiles("**/*.{js,ts,jsx,tsx,java,py,html,css,cpp,c,cs,php,rb,go}", "**/node_modules/**");
            const results = [];
            for (const file of files) {
            ...
            if (selection) {
                const chosen = results.find(x => `${x.filePath} (Line ${x.line + 1})` === selection.label &&
                    x.text === selection.description);
                if (chosen) {
                    const doc = yield vscode.workspace.openTextDocument(chosen.filePath);
    ...
    // 6) Insert Try/Catch
    const insertTryCatchCommand = vscode.commands.registerCommand("projectHelper.insertTryCatchSnippet", () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage("No active editor found!");
            return;
    ...
    // 7) Format Document
    const formatDocumentCommand = vscode.commands.registerCommand("projectHelper.formatDocument", () => __awaiter(this, void 0, void 0, function* () {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            vscode.window.showErrorMessage("No active editor found!");
            return;
    ...
    // 8) Open Docs
    const openDocsCommand = vscode.commands.registerCommand("projectHelper.openDocs", () => __awaiter(this, void 0, void 0, function* () {
        const docsLink = "https://code.visualstudio.com/api";
        yield vscode.env.openExternal(vscode.Uri.parse(docsLink));
        vscode.window.showInformationMessage(`Opened documentation: ${docsLink}`);
    }));
    // 9) Show Quick Tips
    const showQuickTipsCommand = vscode.commands.registerCommand("projectHelper.showQuickTips", () => {
        const tips = [
    ...
    const javaDependencyGraphCommand = vscode.commands.registerCommand("projectHelper.javaDependencyGraph", () => __awaiter(this, void 0, void 0, function* () {
        try {
            const pomFiles = yield vscode.workspace.findFiles("**/pom.xml", "**/node_modules/**");
            const gradleFiles = yield vscode.workspace.findFiles("**/build.gradle", "**/node_modules/**");
            const dependencies = [];
            if (pomFiles.length > 0) {
                const pomDoc = yield vscode.workspace.openTextDocument(pomFiles[0]);
                const pomText = pomDoc.getText();
                const depBlocks = pomText.match(/<dependency>[\s\S]*?<\/dependency>/g) || [];
                depBlocks.forEach(block => {
                    var _a, _b, _c;
                    const groupId = ((_a = block.match(/<groupId>(.*?)<\/groupId>/)) === null || _a === void 0 ? void 0 : _a[1]) || "";
                    const artifactId = ((_b = block.match(/<artifactId>(.*?)<\/artifactId>/)) === null || _b === void 0 ? void 0 : _b[1]) || "";
                    const version = ((_c = block.match(/<version>(.*?)<\/version>/)) === null || _c === void 0 ? void 0 : _c[1]) || "";
                    dependencies.push({ groupId, artifactId, version });
                });
            }
            else if (gradleFiles.length > 0) {
                const gradleDoc = yield vscode.workspace.openTextDocument(gradleFiles[0]);
                const gradleText = gradleDoc.getText();
                const matches = gradleText.match(/implementation\s+['"](.*?)['"]/g) || [];
                matches.forEach(line => {
                    var _a;
                    const notation = ((_a = line.match(/['"](.*?)['"]/)) === null || _a === void 0 ? void 0 : _a[1]) || "";
                ...
                vscode.window.showWarningMessage("No pom.xml or build.gradle found. (Naive search for Java dependencies.)");
                return;
            }
            ...
            const panel = vscode.window.createWebviewPanel("javaDeps", "Java Dependency Graph", vscode.ViewColumn.One, { enableScripts: true });
    ...
    const gitSuggestionsCommand = vscode.commands.registerCommand("projectHelper.gitSuggestions", () => {
        const panel = vscode.window.createWebviewPanel("gitSuggestions", "Git Commit Suggestions", vscode.ViewColumn.One, { enableScripts: true });
    ...
    // 12) Compare Files
    const compareFilesCommand = vscode.commands.registerCommand("projectHelper.compareFiles", () => __awaiter(this, void 0, void 0, function* () {
        try {
            const fileA = yield pickFile("Select first file to compare");
            if (!fileA) {
                vscode.window.showErrorMessage("No file selected for the first file.");
                return;
            }
            const fileB = yield pickFile("Select second file to compare");
            if (!fileB) {
                vscode.window.showErrorMessage("No file selected for the second file.");
                return;
            }
            yield vscode.commands.executeCommand("vscode.diff", vscode.Uri.file(fileA), vscode.Uri.file(fileB), "File Comparison");
        }
        catch (err) {
    ...
    const openDashboardCommand = vscode.commands.registerCommand("projectHelper.openDashboard", () => {
        const panel = vscode.window.createWebviewPanel("codeAssistDashboard", "CodeAssist Dashboard", vscode.ViewColumn.Beside, // Right column
    ...
    // Subscribe commands
    context.subscriptions.push(gatherCommentsCommand, openNotesCommand, searchLogsCommand, insertLogSnippetCommand, searchTodosCommand, insertTryCatchCommand, formatDocumentCommand, openDocsCommand, showQuickTipsCommand, javaDependencyGraphCommand, gitSuggestionsCommand, compareFilesCommand, openDashboardCommand);
}
 ...
 */
function getCodeAssistDashboardHTML() {
    const commands = [
...
/** Comments WebView */
function getCommentsWebviewContent(comments) {
...
function getFloatingNotesHTML(existingNote) {
...
/** Logs WebView */
function getLogsWebviewContent(logs) {
...
function getJavaDepsWebviewContent(deps) {
...
function getGitSuggestionsWebviewContent() {
    return /* html */ `
  <!DOCTYPE html>
  <html>
    <head>
...
function initHoverProvider(context) {
    const hoverProvider = vscode.languages.registerHoverProvider({ scheme: "file", language: "*" }, {
        provideHover(document, position) {
            ...
            if (word.toUpperCase() === "TODO") {
                return new vscode.Hover(new vscode.MarkdownString("**Remember to finish this task!**"));
            }
            return;
        },
    });
...
function initCodeLensProvider(context) {
    const codeLensProvider = vscode.languages.registerCodeLensProvider({ scheme: "file", language: "*" }, new TodoCodeLensProvider());
